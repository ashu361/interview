var QuestionList = React.createClass({
  ...
  render: function() {
    ...
    var questionNodes = activeQuestions.map(function(question) {
      ...
      return (
        <Question key={question_id} question={question} value={props.response[question_id] ? props.response[question_id]: ''} onChange={handleChange} onRadioChange={handleRadioChange} onEnterPressed={handleEnterPressed}></Question>
      );
    });
    return (<ol id="questionare" className="fs-fields">
      {appQuestionNodes.concat(questionNodes)}</ol>)
  }, 
  ...
  handleApplicationChange: function(model, value) {
    ...
    this.props.onApplicationChange(model, value);
  },
