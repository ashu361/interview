var QuestionForm = React.createClass({
  ...
  loadResponseByApplicationId: function (application_id) {
    $.ajax({
      ...
      type: 'GET',
      success: (function (data) {
        if (data.data && data.data.response) this.setState({ response: data.data.response, response_id: data.data.id });
        addStyle();
      }).bind(this),
      error: (function (xhr, status, err) {
        console.error(this.props.url, status, err.toString());
  ...
  createShopCluesTboApplication: function () {
    ...
    var business_entity_data = { 'name': application_response['name'],
      'primary_contact': {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'h1',
          null,
          'OLA Partner Interview'
        )
      ),
      React.createElement(
        'form',
        { id: 'myform', className: 'fs-form fs-form-full', autoComplete: 'off' },
        React.createElement(QuestionList, { new_application_interview: this.state.new_application_interview, interview: this.state.interview,
        ...
        React.createElement(
          'button',
          { className: 'fs-submit', onClick: this.handleSendClicked },
          'Send Information'
        )
      )
    );
